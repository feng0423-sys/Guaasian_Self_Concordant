...
pn_sparse_mvreg <- function(Xc, Y, lambda_gamma, lambda_Omega,
  ...
  for (m in seq_len(max_iter)) {
    ...
    grad_gamma_curr <- grad_gamma(gamma, Omega, Sxx, Sxy, Oinv = Oinv, beta = beta)
    grad_Omega_curr <- grad_Omega(gamma, Omega, Sxx, Sxy, Syy, Oinv = Oinv, beta = beta)
    grad_dir_ip <- sum(grad_gamma_curr * d_gamma) + sum(grad_Omega_curr * d_Omega)
    pen_prev <- penalty_obj(gamma, Omega, lambda_gamma, lambda_Omega)
    
    ls_halvings <- as.integer(pn_ls_ctrl$max_halving + 1L)
    ...
    best_Oinv <- Oinv
    best_alpha <- 0
    best_ls_halvings <- as.integer(pn_ls_ctrl$max_halving + 1L)
    best_armijo_lhs <- NA_real_
    best_armijo_rhs <- NA_real_
    ...
    for (ls_try in 0:pn_ls_ctrl$max_halving) {
      ...
      Oinv_try <- safe_inv_psd(Omega_try)
      loss_try <- loss_fn(gamma_try, Omega_try, Sxx, Sxy, Syy, n,
                          lambda_gamma, lambda_Omega, Oinv = Oinv_try)
      if (is.finite(loss_try)) {
        pen_try <- penalty_obj(gamma_try, Omega_try, lambda_gamma, lambda_Omega)
        armijo_lhs_try <- loss_try - loss_prev
        armijo_rhs_try <- armijo_c * (pen_try - pen_prev - alpha_try * grad_dir_ip)
        
        if (loss_try < best_loss_try) {
          best_loss_try <- loss_try
          best_gamma <- gamma_try
          best_Omega <- Omega_try
          best_Oinv <- Oinv_try
          best_alpha <- alpha_try
          best_ls_halvings <- as.integer(ls_try)
          best_armijo_lhs <- armijo_lhs_try
          best_armijo_rhs <- armijo_rhs_try
        }
        
        if (is.finite(armijo_rhs_try) && armijo_lhs_try <= armijo_rhs_try + 2e-12) {
          alpha <- alpha_try
          ls_halvings <- as.integer(ls_try)
          ls_success <- TRUE
          gamma_new <- gamma_try
          Omega_new <- Omega_try
          Oinv_new <- Oinv_try
          loss_new <- loss_try
          armijo_lhs_used <- armijo_lhs_try
          armijo_rhs_used <- armijo_rhs_try
          break
        }
      }
    ...
    total_ls_halvings <- ls_halvings
    last_alpha_try <- alpha_try
    
    if (!ls_success) {
      line_search_forced <- TRUE
      if (is.finite(best_loss_try)) {
        alpha <- best_alpha
        ls_halvings <- best_ls_halvings
        gamma_new <- best_gamma
        Omega_new <- best_Omega
        Oinv_new <- best_Oinv
        loss_new <- best_loss_try
        armijo_lhs_used <- best_armijo_lhs
        armijo_rhs_used <- best_armijo_rhs
      } else {
        alpha <- 0
        ls_halvings <- as.integer(pn_ls_ctrl$max_halving + 1L)
        gamma_new <- gamma
