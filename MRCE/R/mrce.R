mrce=function(X,Y, lam1=NULL, lam2=NULL, lam1.vec=NULL, lam2.vec=NULL,
     ...
     tol.in=1e-8, kfold=5, silent=TRUE, eps=1e-5, standardize=FALSE, permute=FALSE,
     folds=NULL)
{
  ...
  out=switch(method, single=compute.mrce(X=X,Y=Y, lam1=lam1, lam2=lam2, tol.out=tol.out, 
                                        ...
                                        cov.maxit=cov.maxit, informed=NULL, 
                                        eps=eps, standardize=standardize),
                     fixed.omega=compute.fixed(X=X,Y=Y, lam2=lam2, omega=omega, 
                                               tol.in=tol.in,
                     ...
                     cv=mrce.cv(X=X, Y=Y, lam.vec.1=lam1.vec, lam.vec.2=lam2.vec, 
                                ...
                                eps=eps, standardize=standardize, permute=permute,
                                folds=folds))
  return(out)  
}
